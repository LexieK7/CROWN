...
class ImageNet(ExtendedVisionDataset):
    ...
    def get_image_data(self, index: int) -> bytes:
        ...
        image_full_path = os.path.join(self.root, filename)
    ...
    def _dump_entries(self) -> None:
        ...
        if split == ImageNet.Split.TEST:
            ...
            class_names_map = {class_id: class_name for class_id, class_name in labels}
            ...
            max_class_id_length = max(len(class_id) for class_id, _ in labels)
            max_class_name_length = max(len(class_name) for _, class_name in labels)
            ...
            for idx, (image_full_path, class_index) in enumerate(dataset.samples):
                actual_index = idx + 1
                image_relpath = os.path.relpath(image_full_path, self.root)
            
                class_id = os.path.split(os.path.dirname(image_relpath))[-1]
                class_name = class_names_map.get(class_id, "")

                entries_array[idx] = (actual_index, class_index, class_id, class_name, image_relpath)
